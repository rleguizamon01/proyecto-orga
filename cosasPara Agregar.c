...
void abortar(int ERROR){
    switch (ERROR){
        ...
        printf("Ocurrio un error, memoria insuficiente");
            break;
        case 3:
        printf("Ocurrio un error, OVERFLOW, parece que la parte entera del numero no puede ser representada con %d digitos, en la base solicitada",precisionEntera);
            break;
    }
    exit(EXIT_FAILURE);
}
    ...
    for(*i=precisionEntera-1;*i>=0 ;(*i)--){
        *numAux=*numeroViejo % *baseDestino;
        if (mostrarPasos && *numeroViejo!=0)
            printf("%lld mod %d = %d y (%lld-%d)/%d = ",*numeroViejo,*baseDestino,*numAux,*numeroViejo,*numAux,*baseDestino);
...
void conversor_base_a_base(char* parteEntera,char* parteFraccional, int *baseOrigen, int *baseDestino){
    ...
    parteEnteraFinal=(void*) malloc (sizeof(char)*precisionEntera);
    ...
    *tamFraccion=precisionFraccional;
    ...
    arreglo_char_to_int(parteEntera,parteEnteraTrabajable,tamEntero);
    arreglo_char_to_int(parteFraccional,parteFraccionalTrabajable,tamFraccion);
    ...
    imprimir_arreglo_char(parteEnteraFinal,tamEntero);
    printf(".");
...
int main(void){
	char numeroChar[precisionEntera]= {'0','0','0','0','6','1', '1', '3', '1', '1'};
	char fraccionChar[precisionFraccional]= {'0', '0', '6', '4', '6'};
    ...
    *base2=9;
    ...
    conversor_base_a_base(numeroChar,fraccionChar,base,base2);
